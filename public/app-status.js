...
`;
    ...
    function _wireCreateModal(){
        ...
        if(subBtn&&!subBtn._v4){subBtn._v4=true;
            subBtn.addEventListener('click',async function(e){
                ...
                try{
                    var newItems=await _buildItems(files,txt,selBg);
    ...
    async function _buildItems(files,txt,bg){
        var items=[];
        ...
        for(var i=0;i<files.length;i++){
            ...
            if(isVid){
                ...
                var url=await _uploadFile(f,'video');
                ...
                var iurl=await _uploadFile(f,'image');
    ...
    async function _uploadFile(file, resourceType){
        ...
        if(!cloud){ _notify('Cloudinary not configured','error'); return ''; }
        var fd=new FormData();
        fd.append('file', file);
        fd.append('upload_preset', preset);
        fd.append('resource_type', resourceType==='video'?'video':'image');
        try{
            var r=await fetch('https://api.cloudinary.com/v1_1/'+cloud+'/'+(resourceType==='video'?'video':'image')+'/upload',{method:'POST',body:fd});
            var d=await r.json();
            return d.secure_url||d.url||'';
        }catch(e){console.error('[Status upload]',e);return '';}
    }
    ...
    function _getVidDur(file){
        return new Promise(function(res){
            ...
            v.onloadedmetadata=function(){res(isFinite(v.duration)?v.duration:0);try{URL.revokeObjectURL(v.src);}catch(e){}};
            v.onerror=function(){res(0);};v.src=URL.createObjectURL(file);
        });
    }
