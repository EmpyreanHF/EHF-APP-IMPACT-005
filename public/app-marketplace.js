...
(function empyreanMarketplaceModule() {
    ...
    document.addEventListener('click', function (e) {
        ...
        /* ── Contact seller / Message Seller button ── */
        var contactBtn = t.closest('.contact-seller-btn, .expand-contact-btn, .vf-chat-seller-btn, .mkt-msg-seller-btn');
        if (contactBtn) {
            ...
            var us = _us();
            /* If owner viewing own listing — show contact panel instead */
            var isOwner = _isAdmin() || (us.id && sellerId && sellerId === us.id);
            if (!isOwner && sellerId) {
                _openMarketChatOverlay(sellerId, sellerName.trim());
            } else if (cardEl) {
                /* Owner: expand static contact info */
                ...
                if (isOpen) {
                    ...
                    directInfo.innerHTML = cName||cPhone||cEmail
                        ? ['<p style="font-weight:700;margin-bottom:8px;">Seller Contact</p>',
                           ...
                           cAddr?'<p>'+_esc(cAddr)+'</p>':''].join('')
                        : '<p>Contact via Messages section.</p>';
