...
(function EmpyreanFixFinal() {
    ...
    (function fixDashboardBizPostsSlider(){
        function _buildPostCard(post,pageName,pageAvatar,pageCover){
            ...
            card.dataset.pageId=post.pageId||post.bizId||'';
            card.style.cssText='flex:0 0 160px;width:160px;border-radius:14px;overflow:hidden;cursor:pointer;'+
                'box-shadow:0 3px 14px rgba(10,14,39,0.12);background:white;'+
                'border:1px solid rgba(10,14,39,0.08);scroll-snap-align:start;'+
            ...
            var coverBg=pageCover?'url(\''+_esc(pageCover)+'\') center/cover no-repeat':'linear-gradient(135deg,#0A0E27 0%,#1B2B8B 100%)';
            var avatarSrc=pageAvatar||('https://ui-avatars.com/api/?name='+encodeURIComponent(pageName||'B')+'&background=1B2B8B&color=fff&size=100');
            /* Fixed height instead of aspect-ratio — safe on all Android browsers */
            var productBox=fm&&!isVid
                ? '<div style="width:100%;height:160px;overflow:hidden;background:#f3f4f6;flex-shrink:0;"><img src="'+_esc(fm)+'" style="width:100%;height:100%;object-fit:cover;display:block;" onerror="this.parentNode.style.display=\'none\'"></div>'
                : fm&&isVid
                ? '<div style="width:100%;height:160px;overflow:hidden;background:#0A0E27;position:relative;flex-shrink:0;"><video src="'+_esc(fm)+'" style="width:100%;height:100%;object-fit:cover;display:block;" muted playsinline preload="metadata"></video><div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;"><div style="width:36px;height:36px;border-radius:50%;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;"><i class="fas fa-play" style="color:white;font-size:0.85rem;margin-left:2px;"></i></div></div></div>'
                : '<div style="width:100%;height:90px;background:linear-gradient(135deg,rgba(27,43,139,0.06),rgba(27,43,139,0.12));display:flex;align-items:center;justify-content:center;flex-shrink:0;"><i class="fas fa-store" style="font-size:1.8rem;color:rgba(27,43,139,0.25);"></i></div>';
            card.innerHTML=
                '<div style="height:60px;background:'+coverBg+';position:relative;flex-shrink:0;">'+
                '<div style="position:absolute;bottom:-16px;left:10px;width:34px;height:34px;border-radius:50%;border:2.5px solid white;overflow:hidden;box-shadow:0 2px 6px rgba(0,0,0,0.2);background:#e8eaf6;">'+
                '<img src="'+_esc(avatarSrc)+'" style="width:100%;height:100%;object-fit:cover;" onerror="this.src=\'https://ui-avatars.com/api/?name=B&background=1B2B8B&color=fff&size=100\'">'+
                '</div></div>'+
                '<div style="padding:20px 10px 6px;display:flex;flex-direction:column;gap:1px;">'+
                '<strong style="font-size:0.78rem;font-weight:800;color:#0A0E27;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+_esc(pageName||'Business')+'</strong>'+
                '<span style="font-size:0.62rem;color:#1B2B8B;font-weight:600;">Business Page</span>'+
                '</div>'+
                productBox+
                (post.text?'<div style="padding:7px 10px 10px;flex:1;"><p style="margin:0;font-size:0.72rem;color:#374151;line-height:1.4;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">'+_esc(post.text)+'</p></div>':'<div style="height:8px;"></div>');
            card.addEventListener('mouseenter',function(){ card.style.transform='translateY(-3px)'; card.style.boxShadow='0 8px 24px rgba(10,14,39,0.16)'; });
            card.addEventListener('mouseleave',function(){ card.style.transform=''; card.style.boxShadow='0 3px 14px rgba(10,14,39,0.12)'; });
            card.addEventListener('click',function(){
                var pid=post.pageId||post.bizId||'';
                if(pid) window._activeBizPageId=pid;
                var pages=(window._firestoreBusinessPages||[]);
                var us=_us(); if(us.businessPage) pages=[us.businessPage].concat(pages);
                var biz=pages.find(function(p){return p.id===pid;});
                if(biz) window._activeBizData=biz;
                if(!biz&&card.dataset.bizData){ try{ window._activeBizData=JSON.parse(card.dataset.bizData); }catch(_e2){} }
                if(typeof window.navigateTo==='function') window.navigateTo('business-page');
                setTimeout(function(){ if(typeof window.renderBusinessPage==='function') window.renderBusinessPage(pid); },120);
            });
            return card;
        }
        ...
        function _loadPosts(){ /* disabled — app-business.js §10 _loadProductCards is the single source of truth */ }
    ...
    (function fixDashboardBizContainerIds() {
        function _bridge() {
            /* If the real containers exist but have the old IDs missing, alias them */
            ...
            /* Create invisible alias proxies only once */
            ...
            /* Ensure the real container is visible */
            realContainer.style.display = 'block';

            /* Patch renderDashboardBusinesses to use the correct real IDs */
            var _origRDB2 = window.renderDashboardBusinesses;
            if (_origRDB2 && !_origRDB2._idPatched) {
                window.renderDashboardBusinesses = function() {
                    /* Temporarily reroute the function to use the real DOM IDs */
                    var fakeContainer = document.getElementById('dashboard-business-container');
                    var fakeSlider    = document.getElementById('dashboard-business-slider');
                    /* Point fake aliases to real elements so _origRDB2 writes into them */
                    /* Instead: just call original and then move children to real slider */
                    if (fakeSlider && realSlider) {
                        try { _origRDB2.apply(this, arguments); } catch(e) {}
                        /* Move any new cards from alias slider to real slider */
                        var kids = Array.from(fakeSlider.children);
                        kids.forEach(function(kid) {
                            /* Avoid duplicating cards */
                            var existingId = kid.dataset && kid.dataset.bizId;
                            if (existingId && realSlider.querySelector('[data-biz-id="' + existingId + '"]')) return;
                            realSlider.appendChild(kid);
                        });
                        /* Remove the placeholder empty card if real content arrived */
                        if (realSlider.children.length > 1) {
                            var empty = document.getElementById('bizposts-empty');
                            if (empty) try { empty.parentNode.removeChild(empty); } catch(_e2) {}
                        }
                        realContainer.style.display = 'block';
                    } else {
                        try { _origRDB2.apply(this, arguments); } catch(e2) {}
                    }
                };
                window.renderDashboardBusinesses._idPatched = true;
            }
        }
        ...
        console.log('[§37] Dashboard biz container ID bridge — alias #dashboard-business-* → real #dashboard-bizposts-*.');
    })();
    ...
    (function fixMessageUploadPersist() {
        ...
        function _uploadFileToCloud(file) {
            return new Promise(function(resolve, reject) {
                ...
                var preset = cfg.preset || cfg.uploadPreset || cfg.upload_preset || 'Empyrean_preset';
    ...
    (function initLikesAndShare() {
        ...
        function _us45() {
            return (typeof window.getCurrentUser === 'function' && window.getCurrentUser()) ||
                   window.currentUser || window._currentUser || { id: null };
        }
        function _isGuest45() {
            var u = _us45();
            return !u || !u.id || !!(window.isGuest);
        }
        ...
        function _col45(card) {
            ...
            if (card && (card.classList.contains('crisis-card') || card.classList.contains('sos-card'))) return 'sos_posts';
